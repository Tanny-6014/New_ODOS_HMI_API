        ...
        public List<Accessory> ProductionInvLength(out string errMsg, out int r_intInvLen, out int r_intProdLen, out int r_intTotBend, out int r_intTotArc, out string bvbs)
        {
            ...
            try
            {
                ...
                using (var sqlConnection = new SqlConnection(connectionString))
                {
                    ...
                    ds = (DataSet)sqlConnection.Query<DataSet>(SystemConstants.USP_GET_PRODUCTIONLENGTH_FORMULA_CUBE, dynamicParameters, commandType: CommandType.StoredProcedure);

                }

                //dbManager.CreateParameters(1);
                //dbManager.AddParameters(0, "@CHRSHAPECODE", this.ShapeCode.Trim());
                            ...
                            else if (dtShapeDetails.Rows[rowinc]["CSD_TYPE"].ToString().ToUpper().Equals("ESPLICE") || dtShapeDetails.Rows[rowinc]["CSD_TYPE"].ToString().ToUpper().Equals("NSPLICE") || dtShapeDetails.Rows[rowinc]["CSD_TYPE"].ToString().ToUpper().Equals("DEXTRA"))
                            {
                                if (parameterValue.ToString() != "")
                                {
                                    ...
                                    using (var sqlConnection = new SqlConnection(connectionString))
                                    {
                                        ...
                                        ds = (DataSet)sqlConnection.Query<DataSet>(SystemConstants.USP_GET_COUPLER_DATA_CUBE, dynamicParameters, commandType: CommandType.StoredProcedure);

                                    }

                                    //dbManager.CreateParameters(1);
                                    //dbManager.AddParameters(0, "@COUPLERWITHDIA", parameterValue.ToString().Trim() + this.dia.ToString());
                                    ...
                                    else if ((dtShapeDetails.Rows[rowinc]["CSD_TYPE"].ToString().ToUpper().Equals("ESPLICE") || dtShapeDetails.Rows[rowinc]["CSD_TYPE"].ToString().ToUpper().Equals("NSPLICE") || dtShapeDetails.Rows[rowinc]["CSD_TYPE"].ToString().ToUpper().Equals("DEXTRA")) && dtShapeDetails.Rows[rowinc]["CSD_PRODLENGTH"].ToString().Trim().ToUpper().Equals("Y"))
                                    {
                                        if (parameterValue.ToString() != "")
                                        {
                                            ...
                                            using (var sqlConnection = new SqlConnection(connectionString))
                                            {
                                                ...
                                                var dynamicParameters = new DynamicParameters();
                                                dynamicParameters.Add("@COUPLERWITHDIA", parameterValue.ToString().Trim() + this.dia.ToString());
                                                dsCouplerVal = (DataSet)sqlConnection.Query<DataSet>(SystemConstants.USP_GET_COUPLER_DATA_CUBE, dynamicParameters, commandType: CommandType.StoredProcedure);
                ...
                using (var sqlConnection = new SqlConnection(connectionString))
                {
                    ...
                    dynamicParameters.Add("@InvLength", r_intInvLen);

                    dsCustomInvLength = (DataSet)sqlConnection.Query<DataSet>(SystemConstants.sp_Check_IsCustomInvCutLength, dynamicParameters, commandType: CommandType.StoredProcedure);

                }
                //dbManager.CreateParameters(2);
                //dbManager.AddParameters(0, "@ShapeID", this.ShapeCode.Trim());
                //dbManager.AddParameters(1, "@InvLength", r_intInvLen);
