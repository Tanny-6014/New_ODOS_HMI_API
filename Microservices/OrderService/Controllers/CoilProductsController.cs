...
namespace OrderService.Controllers
{
    ...
    public class CoilProductsController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        [Route("/Index_coil/{appCustomerCode}/{appProjectCode}/{Username}/{appOrderNo}")]
        public ActionResult Index(string appCustomerCode, string appProjectCode, string Username, string appOrderNo)
        {
            Username = Username;//"vishalw_ttl@natsteel.com.sg";
            ...
            //var lSE = db.OrderProjectSE.Find(lOrderNoT);
            //if (lSE != null)
            //{
            ...
            {
                var lAccess = db.UserAccess.Find(Username, appCustomerCode, appProjectCode);
                if (lAccess != null)
                {
            ...
            var lProdStr = getProjectType(appCustomerCode, appProjectCode);
        ...
        [Route("/getProjectType_Coil/{pCustomerCode}/{pProjectCode}")]
        string getProjectType(string pCustomerCode, string pProjectCode)
        {
            ...
            string lReturn = "REBAR,1,WR,1,COLD,1,PC,1";

            var lProcessObj = new ProcessController();
            if (lProcessObj.OpenNDSConnection(ref lNDSCon) == true)
            {
                //lCmd.CommandText = "SELECT NVL(SUM(K.ytot_rebar),0), " +
                //    "NVL(SUM(K.YTOTAL_WR),0), " +
                //    "NVL(SUM(K.YTOT_COLD_ROLL),0), " +
                //    "NVL(SUM(K.YTOTAL_PCSTRAND),0) " +
                ...
                //        "AND to_date(K.GUEEN,'YYYYMMDD') >= (SYSDATE - 180) ";
                List<string> productcodes = new List<string>();

                List<double> productQty = new List<double>();


                lNDSCmd.CommandText = @"SELECT  PROD_CLASS_CODE,  SUM(CAST(PROD_QTY AS FLOAT)) AS TOTAL_QTY FROM HMIContractItem WHERE CONTRACT_NO 
                        IN (SELECT CM.CONTRACT_NO FROM HMIContractMaster CM INNER JOIN HMIContractProject CP 
                ...
                if (lNDSRst.HasRows)
                {
                    ...
                    while (lNDSRst.Read())
                    {
                        ...
                        if (ProductCode == "DBIC")
                        {
                            rebarQty = lNDSRst.GetValue(1) == DBNull.Value ? 0 : Convert.ToDouble(lNDSRst.GetValue(1));
                        ...
                        if (ProductCode == "CRW")
                        {
                            coldQty = lNDSRst.GetValue(1) == DBNull.Value ? 0 : Convert.ToDouble(lNDSRst.GetValue(1));
                        }
                        if (ProductCode == "WRD")
                        {
                            wrQty = lNDSRst.GetValue(1) == DBNull.Value ? 0 : Convert.ToDouble(lNDSRst.GetValue(1));
                        }
                        if (ProductCode == "TR")
                        {
                            pcQty = lNDSRst.GetValue(1) == DBNull.Value ? 0 : Convert.ToDouble(lNDSRst.GetValue(1));
                        }
                    ...
                    lReturn = "REBAR," + rebarQty.ToString() + ","
                    + "WR," + wrQty.ToString() + ","
                    + "COLD," + coldQty.ToString() + ","
                    + "PC," + pcQty.ToString();
            ...
            public ActionResult Index_old()
            {
                ...
                if (lUserType == "PL" || lUserType == "AD" || lUserType == "PM" || lUserType == "PA" || lUserType == "P1" || lUserType == "P2" || lUserType == "PU" || lUserType == "TE")
                {
                    ...
                    //string lUser = User.Identity.GetUserName();
                    //DateTime lTime = (from p in db.StdSheetJobAdvice
                    //                  where p.UpdateBy == lUser
                    //                  select p.UpdateDate).Max();

                    //var lHis = (from p in db.StdSheetJobAdvice
                    //            where p.UpdateBy == lUser &&
                    //            p.UpdateDate == lTime
                    //            select p).ToList();
                    //if (lHis == null)
                    //{
                    //    lHis.Add(new StdSheetJobAdviceModels());
                    //}
                    //Selected = lHis[0].CustomerCode == h.CustomerCode ? true : false
                ...
                //}
                //ViewBag.CustomerCodeCK = lCustomerCodeD; commented by vishal
                ...
                ViewBag.ProjectCodeCK = lProjectCodeD;*/ //commented by vishal

                return View(content.First());
            }

            [HttpPost]
            [Route("/getStdSheetsBySeries_coil/{SeriesCode}")]
            // ValidateAntiForgeryHeader]
            public ActionResult getStdSheetsBySeries(string SeriesCode)
            {
                var lReturn = new List<StdSheetMasterModels>();

                try
                {
                    lReturn = (from p in db.StdSheetMaster
                               where p.mesh_series == SeriesCode
                               select p).ToList();
                }
                catch (Exception ex)
                {
            ...
            [HttpGet]
            //[ValidateAntiForgeryHeader]
            [Route("/getStandardProducts_coil/{ProdCategory}/{GreenType}")]
            ...
            [HttpPost]
            //[ValidateAntiForgeryHeader]
            [Route("/checkOrderStatus_coil/{CustomerCode}/{ProjectCode}/{JobID}")]
            ...
            [HttpPost]
            //[ValidateAntiForgeryHeader]
            [Route("/deletePODocs_coil/{CustomerCode}/{ProjectCode}/{JobID}/{PODocID}")]
            ...
            [HttpPost]
            //[ValidateAntiForgeryHeader]
            [Route("/downloadPODocs_Coil/{CustomerCode}/{ProjectCode}/{JobID}/{PODocID}")]
            ...
            [HttpPost]
            // [ValidateAntiForgeryHeader]
            [Route("/checkPODocs_coil/{CustomerCode}/{ProjectCode}/{JobID}")]
            ...
            [HttpPost]
            // [ValidateAntiForgeryHeader]
            [Route("/sendOrderSubmittedEmail_coil/{CustomerCode}/{ProjectCode}/{JobID}")]
            ...
            //[HttpPost]
            ////[ValidateAntiForgeryHeader]
            //[Route("/getProject_coil/{CustomerCode}")]
            ...
            //[HttpGet]
            //[Route("/RemoveNonMESHProject_Oracle_coil/{pInputProj}")]
            ...
            [HttpGet]
            [Route("/RemoveNonMESHProject_coil/{pInputProj}")]
            ...
            [HttpGet]
            //[ValidateAntiForgeryHeader]
            [Route("/getProjectDetails_coil/{CustomerCode}/{ProjectCode}/{UserName}")]
            public ActionResult getProjectDetails(string CustomerCode, string ProjectCode,string UserName)
            {
                ...
                else if (lUserType == "PL" || lUserType == "AD" || lUserType == "PM" || lUserType == "PA" || lUserType == "P1" || lUserType == "P2" || lUserType == "PU")
                {
                    lCmd.CommandText = "SELECT " +
                    "CustomerCode, " +
                    "ProjectCode, " +
                ...
                //HttpContext.Response.SetCookie(lProjCookies); commented
            ...
            //[HttpPost]
            //// [ValidateAntiForgeryHeader]
            //[Route("/getOrderList_coil/{CustomerCode}/{ProjectCode}")]
            ...
            //[HttpGet]
            //[Route("/updateOrderStatus_coil/{pCustomerCode}/{pProjectCode}")]
            ...
            [HttpGet]
            [Route("/createJobAdvice_coil/{pCustomerCode}/{pProjectCode}/{pClone}")]
            public int createJobAdvice(string pCustomerCode, string pProjectCode, bool pClone)
            {
                ...
                try
                {
                    ...
                    if (pCustomerCode.Length > 0 && pProjectCode.Length > 0)
                    {
                        ...
                        if (lUserType == "CU" || lUserType == "CA" || lUserType == "CM" || lUserType == "PL" || lUserType == "AD" || lUserType == "PM" || lUserType == "PA" || lUserType == "P1" || lUserType == "P2" || lUserType == "PU")
                        {
            ...
            [HttpPost]
            // [ValidateAntiForgeryHeader]
            [Route("/getOrderListSearch_coil/{CustomerCode}/{ProjectCode}/{RequiredDateFrom}/{RequiredDateTo}/{PONo}")]
            ...
            [HttpPost]
            //[ValidateAntiForgeryHeader]
            [Route("/getOrderDetails_coil/{CustomerCode}/{ProjectCode}/{JobID}")]
            ...
            [HttpPost]
            //[ValidateAntiForgeryHeader]
            [Route("/setCloneOrder_coil/{CustomerCode}/{ProjectCode}/{JobID}/{CloneNo}")]
            ...
            [HttpPost]
            [Route("/SaveJobAdvice_coil/{CustomerCode}/{ProjectCode}/{JobID}/{OrderStatus}/{TotalPcs}/{TotalWeight}")]
            // [ValidateAntiForgeryHeader]
            ...
            [HttpPost]
            [Route("/UpdateSUM_coil/{CustomerCode}/{ProjectCode}/{JobID}")]
            // [ValidateAntiForgeryHeader]
            public async Task<ActionResult> UpdateSUM(string CustomerCode, string ProjectCode, int JobID)
            {
                ...
                try
                {
                    var lTotalWT = await (from p in db.StdProdDetails
                                          where p.CustomerCode == CustomerCode &&
                                          p.ProjectCode == ProjectCode &&
                                          p.JobID == JobID
                                          select p.order_wt).SumAsync();

                    var lTotalPCs = await (from p in db.StdProdDetails
                                           where p.CustomerCode == CustomerCode &&
                                           p.ProjectCode == ProjectCode &&
                                           p.JobID == JobID
                                           select p.order_pcs).SumAsync();

                    var oldJobAdvice = await db.StdSheetJobAdvice.FindAsync(CustomerCode, ProjectCode, JobID);
                    var lNewJobAdv = oldJobAdvice;

                    lNewJobAdv.TotalWeight = lTotalWT;
                    lNewJobAdv.TotalPcs = lTotalPCs;
            ...
            [HttpPost]
            // [ValidateAntiForgeryHeader]
            [Route("/OrderWithdraw_coil/{CustomerCode}/{ProjectCode}/{JobID}")]
            ...
            [HttpPost]
            // [ValidateAntiForgeryHeader]
            [Route("/getBBS_coil/{CustomerCode}/{ProjectCode}/{JobID}")]
            ...
            [HttpPost]
            //  [ValidateAntiForgeryHeader]
            [Route("/saveBBS_coil/{CustomerCode}/{ProjectCode}/{JobID}/{StdProdModels}")]
            public ActionResult saveBBS(string CustomerCode, string ProjectCode, int JobID,
                List<StdProdDetailsModels> StdProdModels)
            {
                ...
                try
                {
                    ...
                    if (StdProdModels != null && StdProdModels.Count > 0)
                    {
                        for (int i = 0; i < StdProdModels.Count; i++)
                        {
                            StdProdModels[i].SSID = i + 1;
                            StdProdModels[i].UpdateBy = User.Identity.Name;
                            StdProdModels[i].UpdateDate = DateTime.Now;
                            StdProdModels[i].CustomerCode = CustomerCode;
                            StdProdModels[i].ProjectCode = ProjectCode;
                            StdProdModels[i].JobID = JobID;
                            db.StdProdDetails.Add(StdProdModels[i]);
                        }
                    }
                    db.SaveChanges();
                    Message = "Data has been saved successfully.";
            ...
            [HttpPost]
            //[ValidateAntiForgeryHeader]
            [Route("/ValidatePONumber_coil/{CustomerCode}/{ProjectCode}/{JobID}/{PONumber}")]
            ...
            [HttpPost]
            // [ValidateAntiForgeryHeader]
            [Route("/printOrderDetail_coil/{CustomerCode}/{ProjectCode}/{JobID}")]
