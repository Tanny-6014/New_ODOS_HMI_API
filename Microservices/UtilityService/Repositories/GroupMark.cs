...
namespace UtilityService.Repositories
{
    public class GroupMark
    {
        ...
        public List<GroupMark> SavePRCGroupMarkingDetails()                                              // Save PRC Group Marking Details...
        {
                ...
                using (var sqlConnection = new SqlConnection(connectionString))
                {
                    ...
                    if (dsCustContProjByGroupID != null && dsCustContProjByGroupID.Tables.Count != 0)
                    {
                        foreach (DataRowView drvBeam in dsCustContProjByGroupID.Tables[0].DefaultView)
                        {
                            ...
                            {
                                GroupMarkId = Convert.ToInt32(drvBeam["INTGROUPMARKID"]),
                                SAPMaterial = new SAPMaterial { MaterialCodeID = Convert.ToInt32(drvBeam["INTSAPMATERIALCODEID"]) },
                                ...
                                ParentStructureMarkId = Convert.ToInt32(drvBeam["INTPARENTSTRUCTUREMARKID"])
                            };
        ...
        public string CopyGM_CopyGroupMarking_New(int sourceProjectId, int destProjectId, int sourceGroupMarkId, string destGroupMarkName, int sourceParameterSet, int destParameterSet, string copyFrom, string wbsElements, int IsGroupMarkRevision)
        {
            ...
            try
            {
                using (var sqlConnection = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand cmd = new SqlCommand(SystemConstant.CopyGM_CopyGroupMarking_Insert_New_CUBE, sqlConnection);
                    cmd.CommandType = CommandType.StoredProcedure;
                
                    cmd.Parameters.Add(new SqlParameter("@STRUCTUREELEMENTTYPEID", this.StructureElementTypeId));
                    cmd.Parameters.Add(new SqlParameter("@PRODUCTTYPEID", this.SitProductTypeId));
                    cmd.Parameters.Add(new SqlParameter("@WBSTYPEID", this.WBSTypeId));
                    cmd.Parameters.Add(new SqlParameter("@SOURCEPROJECTID", sourceProjectId));
                    cmd.Parameters.Add(new SqlParameter("@DESTPROJECTID", destProjectId));
                    cmd.Parameters.Add(new SqlParameter("@SOURCEGROUPMARKID", sourceGroupMarkId));
                    cmd.Parameters.Add(new SqlParameter("@DESTGROUPMARKNAME", destGroupMarkName));
                    cmd.Parameters.Add(new SqlParameter("@SOURCEPARAMETERSETID", sourceParameterSet));
                    cmd.Parameters.Add(new SqlParameter("@DESTPARAMETERSETID", destParameterSet.ToString()));
                    cmd.Parameters.Add(new SqlParameter("@COPYFROM", copyFrom));
                    cmd.Parameters.Add(new SqlParameter("@WBSELEMENTIDS", wbsElements));
                    cmd.Parameters.Add(new SqlParameter("@ISGROUPMARKREVISION", IsGroupMarkRevision));
                    cmd.Parameters.Add(new SqlParameter("@USERID", this.CreatedUserId));
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    adapter.Fill(dsCopyGM);
                    var a = dsCopyGM.Tables[1];
                    if (DestinationGroupMarkDto.Count() > 0)
                    {
                        DataTable dt = ConvertToDataTable.ToDataTable(DestinationGroupMarkDto);
                        dsCopyGM.Tables.Add(dt);
        ...
        public int CheckGroupmarkExists(CheckGroupmarkExistDto checkGroupmarkExist)
        {
            int Output = 0;
            try
            {
                ...
                using (SqlConnection sqlConnection = new SqlConnection(connectionString))
                {
                    sqlConnection.Open();
                    var dynamicParameters = new DynamicParameters();
                    dynamicParameters.Add("@INTPROJECTID", checkGroupmarkExist.PROJECTID);
                    dynamicParameters.Add("@INTSTRUCTUREELEMENTTYPEID", checkGroupmarkExist.STRUCTUREELEMENTTYPEID);
                    dynamicParameters.Add("@SITPRODUCTTYPEID", checkGroupmarkExist.PRODUCTTYPEID);
                    dynamicParameters.Add("@GROUPMARKINGNAME", checkGroupmarkExist.GROUPMARKINGNAME);
                    Output = sqlConnection.QueryFirstOrDefault<int>(SystemConstant.usp_CopyGMGroupmarking_CheckGmName, dynamicParameters, commandType: CommandType.StoredProcedure);
                    sqlConnection.Close();
                    return Output;
